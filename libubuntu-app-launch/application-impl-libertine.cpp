	...
	_appname(appname)
{
	...
	auto container_app_path = g_build_filename(container_path, "usr", "share", "applications", (appname.value() + ".desktop").c_str(), NULL);

	if (g_file_test(container_app_path, G_FILE_TEST_EXISTS)) {
		_appinfo = std::shared_ptr<GDesktopAppInfo>(g_desktop_app_info_new_from_filename(container_app_path), [](GDesktopAppInfo * info) { g_clear_object(&info); });
	} else {
		auto home_path = libertine_container_home_path(container.value().c_str());
		auto home_app_path = g_build_filename(home_path, ".local", "share", "applications", (appname.value() + ".desktop").c_str(), NULL);

		if (g_file_test(home_app_path, G_FILE_TEST_EXISTS)) {
			_appinfo = std::shared_ptr<GDesktopAppInfo>(g_desktop_app_info_new_from_filename(home_app_path), [](GDesktopAppInfo * info) { g_clear_object(&info); });
		}
