...
public: /* all one file, no one to hide from */
    ...
    BaseInstance(const AppID& appid,
...
{
    return _registry->impl->thread.executeOnThread<std::vector<std::shared_ptr<Helper::Instance>>>([this]() {
        std::vector<std::shared_ptr<Helper::Instance>> vect;
        auto instances = ubuntu_app_launch_list_helper_instances(_type.value().c_str(), ((std::string)_appid).c_str());
        for (int i = 0; instances[i] != nullptr; i++)
        {
            auto inst = std::make_shared<BaseInstance>(_appid, _type, instances[i], _registry);
            vect.push_back(inst);
        }
...
std::shared_ptr<Helper::Instance> Base::launch(std::vector<Helper::URL> urls)
{
    ...
    return _registry->impl->thread.executeOnThread<std::shared_ptr<BaseInstance>>([this, urlstrv]() {
        auto instanceid = ubuntu_app_launch_start_multiple_helper(_type.value().c_str(), ((std::string)_appid).c_str(),
                                                                  urlstrv.get());
        auto ret = std::make_shared<BaseInstance>(_appid, _type, instanceid, _registry);
        g_free(instanceid);
        return ret;
    });
}
...
std::shared_ptr<Helper::Instance> Base::launch(MirPromptSession* session, std::vector<Helper::URL> urls)
{
    ...
    return _registry->impl->thread.executeOnThread<std::shared_ptr<BaseInstance>>([this, session, urlstrv]() {
        auto instanceid = ubuntu_app_launch_start_session_helper(_type.value().c_str(), session,
                                                                 ((std::string)_appid).c_str(), urlstrv.get());

        return std::make_shared<BaseInstance>(_appid, _type, instanceid, _registry);
    });
}

std::list<std::shared_ptr<Helper>> Base::running(Helper::Type type, std::shared_ptr<Registry> registry)
{
    return registry->impl->thread.executeOnThread<std::list<std::shared_ptr<Helper>>>([type, registry]() {
        ...
        {
            auto helper = std::make_shared<Base>(type, AppID::parse(appidv[i]), registry);
